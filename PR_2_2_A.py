...
for i in range(0, 52*7, 7): #0..7..14..51*7
    idx =int(i /7)
    idx2 = int(i+7)
    m_i[:,idx] = np.mean(training[:,i:idx2],axis=1) #m_i 2576*52
...
for i in range(0, 52*7, 7):
     x_sub = training[:,i:int(i+7)] - np.array([m_i[:,int(i/7)]]*7).T #x_sub 2576*7, x_sub at col 0 is x1-m1
     for j in range (7):
...
for i in range (156): #for each element in the test set
    ...
    confusion_matrix[int(class_prediction), int(class_reference)] += 1 #for the confusion matrix only
